<?php
  ...
  function compute_elo_v2($j1, $j2, $j3, $j4, $data) {
    /*
     * Match classement si au moins un joueur n'est pas classé. Sinon: match elo
     *
     * Match classement:
    ...
    $delta = diffElo($eloV, $eloD);
    if ($j2 > 0) {
      $data[$j1]["delta_elo"] = $delta;
      $data[$j2]["delta_elo"] = $delta;
    } else {
      $data[$j1]["delta_elo"] = 2*$delta;
    }

    if ($j4 > 0) {
      $data[$j3]["delta_elo"] = -$delta;
      $data[$j4]["delta_elo"] = -$delta;
    } else {
      $data[$j3]["delta_elo"] = -2*$delta;
    }
    foreach ([
      ["p"=>$j1, "w"=>1],
      ["p"=>$j2, "w"=>1],
